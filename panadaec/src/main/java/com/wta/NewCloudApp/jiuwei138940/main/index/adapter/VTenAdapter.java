...
public class VTenAdapter extends DelegateAdapter.Adapter<VTenAdapter.ViewHolder> {

    private IndexBean.InfoBean.DataBean infoBean;
    private Context mContext;
    private int type;

    public VTenAdapter(Context mContext, IndexBean.InfoBean.DataBean infoBean, int type) {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        if(getItemViewType(position)==type) {
            ...
            holder.mRecyclerView.setAdapter(new HomeTenAdapter(infoBean.getCellData()));
        }
    }
