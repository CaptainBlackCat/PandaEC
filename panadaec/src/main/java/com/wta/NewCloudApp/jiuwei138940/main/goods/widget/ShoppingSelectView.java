    ...
    public void getView() {
        ...
        for (BigClassification attr : list) {
            ...
            for (int i = 0; i < attr.getList().size(); i++) {
                ...
                button.setBackgroundResource(R.drawable.detail_sx_item);
                button.setButtonDrawable(android.R.color.transparent);
                button.setText(smallAttr.getName());
                button.setTextColor(getResources().getColorStateList(R.color.goods_choice_tv));
                /**
                 * 开放view的位置
                 */
                layout.addView(button, GroupPosition, i);
            }
            addView(layout);
            GroupPosition++;
        }
