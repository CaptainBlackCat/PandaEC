 ...
 * ================================================
 */
public abstract class JsonTokenCallback<T> extends AbsCallback<T> {

    private Type type;
    private Class<T> clazz;
    private LatteDelegate latteDelegate;

    public JsonTokenCallback(LatteDelegate latteDelegate){
        this.latteDelegate=latteDelegate;
    }

    public JsonTokenCallback(Type type) {
        this.type = type;
    }

    public JsonTokenCallback(Class<T> clazz) {
        this.clazz = clazz;
    }
...
//                .params("token", "3215sdf13ad1f65asd4f3ads1f");
        String token = PreferencesUtility.getInstance(Latte.getApplicationContext()).getToken();
        Log.i("token", "showIntegralShop: "+ token);
        if(token==null){
            latteDelegate.getParentDelegate().start(new LoginDelegate());
            return;
        }
